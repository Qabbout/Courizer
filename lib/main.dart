...
class _MyHomePageState extends State<MyHomePage> {
  Chapter? chapter;
  Future<List<Map<String, dynamic>>?>? futureCourses,
      futureChapters,
      futureChapterByCourseCode;
  ...
  void choiceAction(String choice) async {
    if (choice.startsWith(Constants.AddChapters)) {
      ...
      String courseCode = temp[1];
      futureChapterByCourseCode =
          DBProvider.db.getChaptersByCourseCode(courseCode);
      ...
      if (confirmation != null) {
        if (confirmation) {
          ...
          Course? course =
              await DBProvider.db.getCourseByCourseCodeAsCourse(courseCode);

          String name =
              course!.cCode! + "-" + course.cName!.trim().replaceAll(" ", "-");
    ...
    return await showDialog(
        ...
        builder: (BuildContext context) {
          return SimpleDialog(
            ...
            children: <Widget>[
              FutureBuilder<List<Map<String, dynamic>>?>(
                  future: futureChapterByCourseCode,
                  builder: (_, chapterData) {
                              ...
                              child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return SimpleDialogOption(
                                      ...
                                      onPressed: () {
                                        Navigator.pop(context, chapters[index]);
                                      },
                                    );
                                  }),
                            ),
  ...
  Future<File> moveFileCourse(
      File sourceFile, String newPath, Map courseMap) async {
    ...
    ImageCounter counter =
        await DBProvider.db.getCounter("course", course['cCode']);
    String courseName = course['cCode'] + "-" + cName + "-";
    File check =
        File(newPath + "/$courseName" + counter.count.toString() + ".png");

    while (true) {
      if (await check.exists() != true) {
        try {
          final newFile = await sourceFile.copy(
              newPath + "/$courseName" + counter.count.toString() + ".png");
  ...
  Future<File> moveFileCourseAndChapter(File sourceFile, String newPath,
      Map courseMap, String chapterName) async {
    ...
    String cName = course["cName"];
    cName = cName.trim().replaceAll(" ", "-");
    ImageCounter counter = await DBProvider.db
        .getCounterChapter("chapter", course['cCode'], chapterName);
    String courseName = course['cCode'] + "-" + cName + "-";
    File check = File(newPath +
        "/$courseName" +
    ...
    while (true) {
      if (await check.exists() != true) {
        ...
        await DBProvider.db
            .addNewCountChapter("chapter", course['cCode'], chapterName);
        counter = await DBProvider.db
            .getCounterChapter("chapter", course['cCode'], chapterName);
        check = File(newPath +
            "/$courseName" +
            counter.count.toString() +
              ...
              child: FloatingActionButton(
                  ...
                  onPressed: () async {
                    ...
                    if (course != null) {
                      ...
                      if (confirmation != null) if (confirmation) {
                        futureChapterByCourseCode = DBProvider.db
                            .getChaptersByCourseCode(course["cCode"]);
                        Map? chapter = await _chapterDialog(context);
                        if (chapter != null) {
                          await getImage();
                          if (_image != null) {
                            Directory appDocDirectory =
                                await getApplicationDocumentsDirectory();
                            List listOfDirectories = appDocDirectory.listSync(
                                recursive: true, followLinks: true);
                            List<String> directories = [];
                            listOfDirectories.forEach((element) {
                              directories.add(element.path);
                            });
                            for (var i = 0; i < directories.length; i++) {
                              if (directories[i].contains(course["cCode"])) {
                                File img = await moveFileCourseAndChapter(
                                    _image!,
                                    directories[i],
                                    course,
                                    chapter["name"]);
                        ...
                        if (_image != null) {
                          ...
                          for (var i = 0; i < directories.length; i++) {
                            if (directories[i].contains(course["cCode"])) {
                              File img = await moveFileCourse(
                                  _image!, directories[i], course);
                              _image = null;
                              img.createSync();
                              break;
                            }
