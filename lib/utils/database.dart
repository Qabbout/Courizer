...
class DBProvider {
  ...
  initDB() async {
    return await openDatabase(join(await getDatabasesPath(), "courizer.db"),
  ...
  Future<dynamic> getCounter(String type, String code) async {
    ...
    List<Map> res = await db.rawQuery(
        "SELECT * FROM counter WHERE type = ? AND code = ?", [type, code]);
    return ImageCounter(count: res.length + 1, type: type);
  }

  Future<dynamic> getCounterChapter(
      String type, String code, String chapName) async {
    final db = await database;
    List<Map> res = await db.rawQuery(
        "SELECT * FROM counter WHERE type = ? AND code = ? AND chapName = ?",
        [type, code, chapName]);
    return ImageCounter(count: res.length + 1, type: type);
  }
  ...
  Future<dynamic> addNewCount(String type, String code) async {
    final db = await database;
    var res = await db
        .rawInsert("INSERT INTO counter(type, code) VALUES(?,?)", [type, code]);
    return res;
  }

  Future<dynamic> addNewCountChapter(
      String type, String code, String chapName) async {
    final db = await database;
    var res = await db.rawInsert(
        "INSERT INTO counter(type, code, chapName) VALUES(?,?,?)",
        [type, code, chapName]);
    return res;
  }
  ...
  Future<List<Map<String, dynamic>>?>? getChaptersByCourseCode(
      courseCode) async {
    final db = await database;
    var res = await db
        .rawQuery('SELECT * FROM chapter WHERE courseCode = ?', [courseCode]);
    if (res.length == 0) return null;
    return res;
  }
  ...
  Future<List<Map<String, dynamic>>?>? getCourseByCourseCode(courseCode) async {
    final db = await database;
    var res =
        await db.rawQuery('SELECT * FROM course WHERE cCode = ?', [courseCode]);
    if (res.length == 0) return null;
    return res;
  }
  ...
  Future? deleteChapterByName(name) async {
    final db = await database;
    await db.rawDelete('DELETE FROM chapter WHERE name = ?', [name]);
  }

  Future? deleteCourserByCode(courseCode) async {
    final db = await database;

    await db.rawDelete('DELETE FROM counter WHERE code = ?', [courseCode]);

    await db
        .rawDelete('DELETE FROM chapter WHERE courseCode = ?', [courseCode]);
    await db.rawDelete('DELETE FROM course WHERE cCode = ?', [courseCode]);
  }
}
