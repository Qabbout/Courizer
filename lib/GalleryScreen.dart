...
class _GalleryState extends State<Gallery> {
  ...
  Widget build(BuildContext context) {
    ...
    return images.length !=0 ? Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          ...
          IconButton(icon: Icon(Icons.file_upload) ,tooltip: "Export this course images as a zip file", onPressed: () async {
            Course course = await DBProvider.db.getCourseByCourseCodeAsCourse(widget.courseCode);
         String name = course.cCode +"-" + course.cName.trim().replaceAll(" ", "-");
      Directory appDocDirectory = await getApplicationDocumentsDirectory();
      var dataDir = Directory("${appDocDirectory.path}/" + name);
  try {
    final zipFile = File(appDocDirectory.path + "/${widget.courseCode}.zip");
          ...
          return GestureDetector(
            onTap:()async { 
                await Navigator.push(
                  ...
                  MaterialPageRoute(builder: (context) => GalleryPage(imageList: images, indexx: index,)),
        ...
        body: Center(
          child: Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Icon(Icons.sentiment_dissatisfied, color: Colors.deepPurple, size: 120,)
